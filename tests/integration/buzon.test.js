...
// buzonRateLimit es un singleton a nivel de módulo (no se recrea por test como
// en app.js), así que cada test usa un X-Forwarded-For distinto para no pisar
// el conteo de otros — salvo el test que prueba el límite a propósito
let contadorIp = 0;
...
describe('POST /api/buzon — pública', () => {
  ...
  it('notifica al superadmin cuando llega un mensaje (si existe uno)', async () => {
    ...
    await esperarLlamada(nodemailerSendMailMock);
  ...
  it('acepta un teléfono sin +57 y lo normaliza', async () => {
    const res = await request(app).post('/api/buzon').set('X-Forwarded-For', ipDePrueba()).send({
      nombre: 'Ana', correo: 'x@test.edumon.com', telefono: '3001112233', mensaje: 'Mensaje con longitud suficiente',
    });
    expect(res.status).toBe(201);
